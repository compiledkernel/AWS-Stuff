...
foreach $mount (@fstab) {
	...
	for($i = 0; $i < length($mount) - 4; ++$i) {
		$usermountable = 1 if (substr($mount,$i,4) eq "user");
	}
...
# you may email the author at sildan@inbox.com or visit my website at www.sildan.tk
